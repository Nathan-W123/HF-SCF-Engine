...
`media/hero.png` (1920×1080) and `media/hero.mp4` (1920×1080, 30 fps) are
generated by `make_hero.py` entirely from the logged `swap` run. There is no
matplotlib anywhere in that path.

The renderer is a small custom one:

1. a real pinhole perspective camera in numpy (with roll, so the swarm's
   vertical stack can be laid diagonally across the frame);
...
3. each aircraft is a real 3-D mesh — swept wings, fuselage spine and keel,
   ...
   depth-sorted facet by facet, flat-shaded against a fixed key light, and
   rasterised with PIL polygon fill. Because the facet normals genuinely differ,
   bank angle reads: as an aircraft rolls its wings swing through the light while
   the fin does the opposite;
